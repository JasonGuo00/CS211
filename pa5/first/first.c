...
void initiateMatrix(int columns, int rows, int table[rows][columns]) {
    for(int i = 0; i < rows; i++) {
        char string[columns+1];
        getBinary(columns, i, string);
        for(int j = 0; j < columns; j++) {
            table[i][j] = string[j]-48;
        }
    }
...
void DECODER(int num_Inputs, int num_Outputs, char** inputs, char** outputs, bool compare) {
    int table[num_Outputs][num_Inputs];
...
void MUX(int num_Inputs, int num_Selectors, char** inputs, char** var_selectors, char name_output[50], bool compare) {
    ...
    for(int i = 0; i < sizeOutputs; i++ ) {
        ...
        int x = bin2dec(num_Selectors, string);
        if(variables) {
...
int main(int argc, char* argv[]) {
    ...
    int table[sizeOutputs][numInputs];
    initiateMatrix(numInputs, sizeOutputs, table);
